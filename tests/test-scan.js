...
const {
    ...
    buildKeys, keyIndexOf, markerClips, makeRegistry, TOL
} = require('../src/core/scan.js');
...
ok('finds a sync fully inside a tile', () => {
    const d = blank();
    plantSync(d, 500, 500);
    assert.deepStrictEqual(scanSync(d, W, H, RGB), [500, 500]);
});

ok('finds a sync at the maximum in-tile offset', () => {
    const d = blank();
    plantSync(d, 996, 7);
    assert.deepStrictEqual(scanSync(d, W, H, RGB), [996, 7]);
});
...
for (const start of [997, 998, 999]) {
    ok('seam detection for sync starting at ' + start, () => {
        ...
        assert.deepStrictEqual(seamCheck(ea.right, eb.left, H, W), [start, 300]);
    });
}
...
console.log('test-scan: ' + passed + ' test groups passed, keyspace=' + space.valid.length);